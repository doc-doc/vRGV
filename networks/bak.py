...
class AttHierarchicalGround(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, visual_dim, word_dim, num_layers=1):
        ...
        self.embedding_word = nn.Sequential(nn.Linear(word_dim, self.embed_dim),
         ...
         #########for 1st stage training ###########################
        self.embedding_visual = nn.Sequential(nn.Linear(visual_dim-5, hidden_size),
                                       nn.ReLU(),
                                       nn.Dropout(dropout))

        self.embedding_location = nn.Sequential(nn.Linear(5, hidden_size),
                                              nn.ReLU(),
                                              nn.Dropout(dropout))
    ...
    def spatialAtt(self, videos, relations):
        ...
        frame_feat = video_embeds.sum(dim=2)
        sub_satt_values, obj_satt_values = None, None
        #####################for 1st stage training######################
